...
def compareECGfigure_old(threadsNum_val, meshName, conduction_speeds, resolution_list, figPath, healthy_val,
        ...
        if resolution_count > 0:
            ...
            for resolution_i in range(len(aux_resolution_list)):
                ...
                if os.path.isfile(fileName):
                    ...
                    rootNodeActivationTimes = np.zeros(rootNodeActivationIndexes.shape) # 02/12/2021 Tested how many decimals were needed to get the # TODO: THIS IS NOT THE GOOD CODE, IT'S IGNORING THE ROOT NODE
                    ...
                    with open(figPath + meshName + '_' + resolution + '_available_root_nodes.csv', 'w') as f:
                    ...
                    print('best_id')
                    print(best_id)
                    print('best CC params')
                    print(population_original[best_id, :])
                    ...
                    with open(figPath + meshName + '_' + resolution + '_' + str(consistency_i) + '_inferred_root_nodes.csv', 'w') as f:
                        f.write('"Points:0","Points:1","Points:2"\n')
                    ...
                    for i in range(nb_leads):
                        ...
                        axs_res.plot(best_ecg[i, :], 'r-', label='Best-'+resolution, linewidth=1.2) # 2021/12/15 - TODO Comment
            ...
            for resolution_i in range(len(aux_resolution_list)):
                ...
                if os.path.isfile(fileName):
                    ...
                    for i in range(nb_leads):
                        ...
                        # axs_res.plot(best_ecg_CC[i, :], 'b-', label='Best-CC', linewidth=1.2) # 2021/12/15 - TODO Comment                        # axs_res.plot(best_ecg[i, :], color='purple', linestyle='-', label='Best-'+resolution, linewidth=1.2) # 2021/12/15 - TODO Uncomment
                        axs_res.plot(best_ecg_CC[i, :], 'b-', label='Simulated', linewidth=1.2) # 2021/12/15 - TODO Comment                        # axs_res.plot(best_ecg[i, :], color='purple',
                        # linestyle='-', label='Best-'+resolution, linewidth=1.2) # 2021/12/15 - TODO Uncomment
                        # for j in range(prediction_list.shape[0]):
                        #     prediction_lead = prediction_list[j, i, :]
                        #     lead_size = np.sum(np.logical_not(np.isnan(prediction_list[j, i, :])))
                        #     prediction_lead = prediction_lead[:lead_size]
...
#add a little of buffer time
    ...
    # plt.hist(not_used_pk_times_aux)
    # print()
    # rootNodeActivationTimes = np.around(np.concatenate((lv_PK_time_mat[lvActnode_ids], rv_PK_time_mat[rvActnode_ids]), axis=0), decimals=4) # 02/12/2021 Tested how many decimals were needed to get the
    rootNodeActivationTimes = np.zeros(rootNodeActivationIndexes.shape) # 02/12/2021 Tested how many decimals were needed to get the # TODO: THIS IS NOT THE GOOD CODE, IT'S IGNORING THE ROOT NODE
    # TIMES
...
#         part_theta[:, scale_param] = np.exp(part_theta[:, scale_param])
    ...
    def sample(self, targetDiscrepancy):
        ...
        visualise_count_ini = 10000 # zero will print at every iteration
